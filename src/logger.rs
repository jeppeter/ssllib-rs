...
macro_rules! ssllib_format_buffer_log {
	($buf:expr,$len:expr,$info:tt,$iv:expr,$($arg:tt)+) => {
		...
		ssllib_debug_out($iv,&c);
	}
}

#[macro_export]
macro_rules! ssllib_buffer_error {
	($buf:expr,$len:expr,$($arg:tt)+) => {
		ssllib_format_buffer_log!($buf,$len,"<ERROR>",0,$($arg)+);
	}
}

#[macro_export]
macro_rules! ssllib_buffer_warn {
	($buf:expr,$len:expr,$($arg:tt)+) => {
		ssllib_format_buffer_log!($buf,$len,"<WARN>",1,$($arg)+);
	}
}

#[macro_export]
macro_rules! ssllib_buffer_info {
	($buf:expr,$len:expr,$($arg:tt)+) => {
		ssllib_format_buffer_log!($buf,$len,"<INFO>",2,$($arg)+);
	}
}

#[macro_export]
macro_rules! ssllib_buffer_debug {
	($buf:expr,$len:expr,$($arg:tt)+) => {
		ssllib_format_buffer_log!($buf,$len,"<DEBUG>",3,$($arg)+);
	}
}

#[macro_export]
macro_rules! ssllib_buffer_trace {
	($buf:expr,$len:expr,$($arg:tt)+) => {
		ssllib_format_buffer_log!($buf,$len,"<TRACE>",4,$($arg)+);
	}
}
