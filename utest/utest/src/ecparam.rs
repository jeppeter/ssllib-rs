...
fn eccurvedec_handler(ns :NameSpaceEx,_optargset :Option<Arc<RefCell<dyn ArgSetImpl>>>,_ctx :Option<Arc<RefCell<dyn Any>>>) -> Result<(),Box<dyn Error>> {	
	let sarr :Vec<String>;
	...
	for f in sarr.iter() {
		let code = read_file_bytes(f)?;
		let mut xname = X9_62_CURVE::init_asn1();
		let _ = xname.decode_asn1(&code)?;
		let mut f = std::io::stderr();
		xname.print_asn1("X9_62_CURVE",0,&mut f)?;
		let vcode = xname.encode_asn1()?;
		debug_buffer_trace!(vcode.as_ptr(),vcode.len(),"encode X9_62_CURVE");
	}

	Ok(())
}

fn ecparamsdec_handler(ns :NameSpaceEx,_optargset :Option<Arc<RefCell<dyn ArgSetImpl>>>,_ctx :Option<Arc<RefCell<dyn Any>>>) -> Result<(),Box<dyn Error>> {	
	let sarr :Vec<String>;
	...
	for f in sarr.iter() {
		let code = read_file_bytes(f)?;
		let mut xname = ECPARAMETERS::init_asn1();
		let _ = xname.decode_asn1(&code)?;
		let mut f = std::io::stderr();
		xname.print_asn1("ECPARAMETERS",0,&mut f)?;
		let vcode = xname.encode_asn1()?;
		debug_buffer_trace!(vcode.as_ptr(),vcode.len(),"encode ECPARAMETERS");
	}
...
pub fn load_ecparam_handler(parser :ExtArgsParser) -> Result<(),Box<dyn Error>> {
	let cmdline = r#"
	{
		...
		"eccurvedec<eccurvedec_handler>##binfile ... to decode X9_62_CURVE##" : {
			"$" : "+"
		},
